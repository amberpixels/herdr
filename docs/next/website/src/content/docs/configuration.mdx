...
By default Herdr leaves a blank row between entries in both lists. Set `space_panel_row_gap = false` or `agent_panel_row_gap = false` for a tighter, compact list. Set `agent_panel_numbers = true` to show each agent's position (1-9) under its icon, matching the `focus_agent` shortcuts.
