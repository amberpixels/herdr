...
Set `sidebar_single_line = true` to render each workspace as `name • branch` on one line instead of two. `space_panel_row_gap` and `agent_panel_row_gap` independently toggle the blank row between entries in the spaces and agents lists; set either to `false` for a tighter list. Set `agent_panel_numbers = true` to show each agent's position (1-9) under its icon, matching the `focus_agent` shortcuts.
